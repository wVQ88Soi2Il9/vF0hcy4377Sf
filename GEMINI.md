...
- **邊界隔離與 Import 規範 (Public Entrypoint Rule)**：
  ...
  - **Namespace Import 規範**：引用 `core` 或 `pack`（包含 `world`）時，**只允許使用 `import * as (core/pack)` 命名空間引入**（例：`import * as core from '@/core'`、`import * as vanilla_alpha from '@/packs/vanilla_alpha'`、`import * as world from '@/world'`），嚴禁具名引用（named import）或單獨 `import type`。
