import type { device, vector } from '../core/types'
import { add_vector, rotate_vector_3d } from './math'

/**
 * Gets the world coordinates of all cells occupied by a device.
 */
export function get_world_cells(dev: device): vector[]
{
    return dev.positions.map(pos => 
 ...
 */
export function get_world_ports(dev: device, type: 'input' | 'output'): vector[]
{
    const ports = type === 'input' ? dev.input_ports : dev.output_ports
