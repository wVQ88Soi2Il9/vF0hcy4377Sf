                ...
                for (const raw_recipe of file_data) {
                    ...
                    const processed_recipe: recipe = {
                        ...
                        inputs: raw_recipe.inputs?.map((input: any) => ({
                            ...input,
                            item_id: resolve_id(input.item_id || input.itemId, namespace)
                        })) || [],
                        outputs: raw_recipe.outputs?.map((output: any) => ({
                            ...output,
                            item_id: resolve_id(output.item_id || output.itemId, namespace)
                        })) || []
                    };
                    ...
                    if (raw_recipe.power) {
                        processed_recipe.power = raw_recipe.power.map((p: any) => ({
                            ...p,
                            power_id: resolve_id(p.power_id || p.powerId, namespace)
                        }));
                    }
